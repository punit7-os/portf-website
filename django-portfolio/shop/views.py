...
def signup(request):
    ...
    # ✅ ALWAYS define otp_data first (FIXES ERROR)
    ...
    if request.method == "POST":
        ...
        if request.POST.get("resend_otp") == "1":
            ...
            otp_data["expires_at"] = time.time() + 300
        ...
        if request.POST.get("otp_verify") == "1":
            ...
            return JsonResponse({
                "success": True,
                "redirect_url": reverse("shop:product_list")
            })
        ...
        return JsonResponse({
            "success": True,
            "show_otp": True,
            "otp_sent_to": otp_data["email"]
        })

    # ✅ GET REQUEST (SAFE)
    return render(request, "registration/signup.html", {
        "form": CustomUserCreationForm(),
        "show_otp": bool(otp_data),
        "otp_data": otp_data,
        "otp_sent_to": otp_data["email"] if otp_data else None,
    })

@login_required
def profile(request):
    # ✅ Ensure profile exists (ONLY ONCE, CORRECTLY)
